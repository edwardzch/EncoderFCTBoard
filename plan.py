很好的简化！既然只需要保存
`PA`
参数（用户设置），`DP`（显示参数）不需要保存，那么方案会变得更加高效和轻量。
...
和
Page
127）作为存储区，采用 ** 乒乓（Ping - Pong）机制 ** 来实现追加写入和磨损均衡。
...
Key
4
保存时，查找当前页的 ** 下一个空白位置(0xFF...) **。
2.
写入一条新记录：`ID = 当前参数号, Value = 新值
`。
3. * 优势 *：不擦除，速度极快（微秒级）。
...
static
uint32_t
g_ActivePageAddr = FLASH_PAGE_A_ADDR; // 当前正在使用的页
...
int32_t
save_val = PA_Buffer[save_id]; // 注意：此时
Buffer
已被
EditVal
更新
...
顶部
int32_t
PA_Buffer[PA_SIZE] = {100, 200, 500, ...}; // 你的出厂默认值
...
2. ** 掉电安全 **:
因为我们只保存
`PA`。在搬家时，我们是先写入新页，再擦除旧页。
如果在写入新页时断电 -> 旧页还在，数据不丢。
如果在擦除旧页时断电 -> 新页已经好了，数据不丢。
3. ** 极速响应 **:
平时保存只需要写
8
个字节，耗时几十微秒，数码管显示完全不会闪烁或卡顿。